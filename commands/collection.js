...
function buildPage(targetId, displayName, page, sort) {
  ...
  return { embed, page: p, pages, sort };
}

function buildRows(targetId, viewerId, page, pages, sort) {
  const nav = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId(`collection:prev:${targetId}:${viewerId}:${page}:${sort}`)
      .setLabel('◀')
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(page === 0),

    new ButtonBuilder()
      .setCustomId(`collection:next:${targetId}:${viewerId}:${page}:${sort}`)
      .setLabel('▶')
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(page >= pages - 1),

    new ButtonBuilder()
      .setCustomId(`collection:sort:${targetId}:${viewerId}:0:date`)
      .setLabel('📅 Date')
      .setStyle(sort === 'date' ? ButtonStyle.Primary : ButtonStyle.Secondary),

    new ButtonBuilder()
      .setCustomId(`collection:sort:${targetId}:${viewerId}:0:alpha`)
      .setLabel('🔤 A–Z')
      .setStyle(sort === 'alpha' ? ButtonStyle.Primary : ButtonStyle.Secondary),

    new ButtonBuilder()
      .setCustomId(`collection:sort:${targetId}:${viewerId}:0:rarity`)
      .setLabel('⭐ Rareté')
      .setStyle(sort === 'rarity' ? ButtonStyle.Primary : ButtonStyle.Secondary),
  );
...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const rows = buildRows(target.id, interaction.user.id, page, pages, sort);
  ...
  async handleButton(interaction) {
    const [, action, targetId, viewerId, pageStr, sort] = interaction.customId.split(':');

    if (interaction.user.id !== viewerId) {
      return interaction.reply({
        content: '❌ Ces boutons ne vous appartiennent pas.',
        ephemeral: true
      });
    }
    ...
    await interaction.update({
      embeds: [embed],
      components: buildRows(targetId, viewerId, page, pages, sort)
    });
  },
};